...
Shader "Unlit/ClimatZonesShader"
{
	Properties
	{
		...
		[ASEEnd][ASEBegin]_base_climat_zones("base_climat_zones", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float simplePerlin3D11 = snoise( float3( packedInput.ase_texcoord1.xy ,  0.0 )*22.0 );
				simplePerlin3D11 = simplePerlin3D11*0.5 + 0.5;
				float2 temp_cast_1 = (simplePerlin3D11).xx;
				
				surfaceDescription.Color = tex2D( _base_climat_zones, temp_cast_1 ).rgb;
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float simplePerlin3D11 = snoise( float3( packedInput.ase_texcoord.xy ,  0.0 )*22.0 );
				simplePerlin3D11 = simplePerlin3D11*0.5 + 0.5;
				float2 temp_cast_1 = (simplePerlin3D11).xx;
...
ASEEND*/
//CHKSM=38193FBC1CA79C2245A1E7488D895ED26F961BF0